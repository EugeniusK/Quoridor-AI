...
arr = np.arange(2*81, dtype=np.int8).reshape(81, 2)
def iterate(arr):
    for i in range(81):
        arr[i]
...
@njit
def iterate_jit(arr):
    for action in np.nditer(arr):
        action


iterate_jit(np.array([1, 2, 3]))
"""
...
# """
# print(timeit.timeit("iterate(arr)", setup) / 1000000)

# code = """
# for action in np.nditer(arr):
#     action
# """
# print(timeit.timeit("iterate_jit(arr)", setup) / 1000000)
...
def shift5_numba(arr, num, ):
    ...
    if num > 0:
        ...
        result[:] = arr
    return result
print('numba', shift5_numba(np.arange(1, 9), 2))
print('numpy', np.roll(np.arange(1, 9), 2))

test_arr = np.arange(1,9, dtype=np.int8)
"""
